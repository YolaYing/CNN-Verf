...
//     randomness: Vec<F>,
//     /// Hyrax verifier key (optional, if used with commitments)
//     vk: Option<<HyraxPCS as PolynomialCommitment<F, DenseMultilinearExtension<F>>>::VerifierKey>,
// }
...
//         index_info: &PolynomialInfo,
//         vk: Option<HyraxPCS::VerifierKey>,
//     ) -> VerifierState<F> {
//         VerifierState {
//             round: 1,
...
//         // Step 1: to verify the commitment
//         if !verifier_state.polynomials_received.is_empty() {
//             let last_poly = verifier_state.polynomials_received.last().unwrap();
//             let is_valid = HyraxPCS::verify(
//                 vk,
//                 &[last_poly.clone()],
//                 &prover_msg.evaluations,
//                 &verifier_state.randomness,
//                 None,
//             )
//             .expect("Hyrax commitment verification failed.");

//             if !is_valid {
//                 panic!("Verifier rejected: Commitment verification failed.");
//             }
//         }

//         // Step 2: generate verifier message for next round
...
//         // Step 3: update verifier state
//         if verifier_state.round == verifier_state.nv {
//             verifier_state.finished = true; // mark the verifier as finished
//         } else {
...
//         // use hyrax to verify the commitment
//         if let Some(vk) = &verifier_state.vk {
//             for (i, evaluations) in verifier_state.polynomials_received.iter().enumerate() {
//                 let randomness = &verifier_state.randomness[0..i + 1];
//                 let is_valid = HyraxPCS::verify(
//                     vk,
//                     &[evaluations.clone()],
//                     &randomness,
...
//                     None,
//                 )
//                 .expect("Hyrax verification failed.");

//                 if !is_valid {
//                     return Err(crate::Error::Reject(Some(
//                         "Hyrax commitment verification failed.".into(),
//                     )));
//                 }
//             }
//         }
...
impl<F: Field> IPForMLSumcheck<F> {
    pub fn verifier_init(
        index_info: &PolynomialInfo,
        vk: Option<
            <HyraxPCS as PolynomialCommitment<F, DenseMultilinearExtension<F>>>::VerifierKey,
        >,
    ) -> VerifierState<F> {
        VerifierState {
            round: 1,
    ...
    pub fn verify_round<R: RngCore>(
        prover_msg: ProverMsg<F>,
        verifier_state: &mut VerifierState<F>,
        vk: &<HyraxPCS as PolynomialCommitment<F, DenseMultilinearExtension<F>>>::VerifierKey,
        rng: &mut R,
    ) -> Option<VerifierMsg<F>> {
        if verifier_state.finished {
            panic!("Verifier state is already finished.");
        }
        ...
        if !verifier_state.polynomials_received.is_empty() {
            ...
            let is_valid = HyraxPCS::verify(
                ...
                &[LabeledPolynomial::new(
                    "poly".to_string(),
                    DenseMultilinearExtension::from_evaluations_slice(last_poly.len(), last_poly),
                    None,
                    None,
                )],
                &verifier_state.randomness,
    ...
    pub fn check_and_generate_subclaim(
        ...
        for i in 0..verifier_state.nv {
            ...
            if p0 + p1 != expected {
                return Err(crate::Error::Reject(Some(
                    "Prover message is inconsistent.".into(),
                )));
            }
            expected = interpolate_uni_poly(evaluations, verifier_state.randomness[i]);
        }
